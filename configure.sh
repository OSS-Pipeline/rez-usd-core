...
USD_CORE_VERSION=${REZ_BUILD_PROJECT_VERSION}
...
echo -e "[CONFIGURE][ARGS] USD-CORE VERSION: ${USD_CORE_VERSION}"

# We check if the arguments variables we need are correctly set.
# If not, we abort the process.
if [[ -z ${EXTRACT_PATH} || -z ${BUILD_PATH} || -z ${INSTALL_PATH} || -z ${USD_CORE_VERSION} ]]; then
    echo -e "\n"
...
# We run the configuration script of USD-Core.
echo -e "\n"
echo -e "[CONFIGURE] Running the configuration script from USD-Core-${USD_CORE_VERSION}..."
echo -e "\n"
...
echo -e "\n"
echo -e "[CONFIGURE] Finished configuring USD-Core-${USD_CORE_VERSION}!"
echo -e "\n"
